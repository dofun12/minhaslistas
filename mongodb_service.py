...
class MongoDBService:
    ...
    def add_list_item(self, list_name, item_value):
        self.init_connection()
        list_item = {"id": str(uuid.uuid4()), "value": item_value}
        filter = {"list_name": list_name}
        collection = self.db['lists']
        items = list(collection.find(filter))
        if items is None or len(items) == 0:
            collection.insert_one({"list_name": list_name, "list_items": list_item})
            self.client.close()
            return
        collection.update_one(filter, {"$push": {"list_items": list_item}})
        self.client.close()
